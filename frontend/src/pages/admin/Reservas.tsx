...
export default function Reservas() {
    ...
    async function alterarStatus(
        ...
        try {
            ...
            setErro(error.response?.data?.message || "Erro ao alterar reserva");
        }
    }
        ...
        <div className="p-8 bg-slate-100 min-h-screen">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => navigate("/admin")}
                    className="flex items-center gap-2 bg-slate-700 text-white px-5 py-3 rounded-xl ">
                    <ArrowLeft size={20} />
                    Voltar
                </button>
            </div>
                ...
                <table className="w-full">
                    <thead className="bg-slate-100">
                        <tr>
                            ...
                            <th className="p-4 text-left">Espaço</th>
                            ...
                            reservas.length === 0 ? (
                                ...
                                reservas.map(reserva => (
                                    <tr key={reserva.id} className="border-t">
                                        ...
                                        <td className="p-4">{reserva.solicitante?.nome}</td>
                                        <td className="p-4">{reserva.espaco?.nome}</td>
                                        <td className="p-4"> {formatarData(reserva.dataInicio)}</td>
                                        <td className="p-4"> {formatarData(reserva.dataFim)} </td>
                                        <td className="p-4"> {reserva.status} </td>
                                        <td className="p-4"> {reserva.motivo || "-"} </td>
