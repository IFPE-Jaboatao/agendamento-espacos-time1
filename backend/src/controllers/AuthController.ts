...
export class AuthController {
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      if (error) {
        logger.warn('Tentativa de login com dados inválidos', { ip: req.ip, userAgent: req.get('User-Agent'), details: error.details[0].message });
  ...
  async registrar(req: Request, res: Response) {
    try {
      ...
      if (error) {
        logger.warn('Tentativa de registro com dados inválidos', { ip: req.ip, userAgent: req.get('User-Agent'), details: error.details[0].message });
        return res.status(400).json({ message: "Dados inválidos", details: error.details[0].message });
      }

      const usuario = await service.registrar(value);
      logger.info('Usuário registrado', { userId: usuario.id, login: value.login, perfil: value.perfil, ip: req.ip });
      return res.status(201).json({ id: usuario.id, nome: usuario.nome, email: usuario.email, perfil: usuario.perfil });
    } catch (err: any) {
      logger.error('Erro no registro', { login: req.body.login, ip: req.ip, error: err.message });
